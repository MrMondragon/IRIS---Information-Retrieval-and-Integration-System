  ...
  [Serializable]
	public class LALRParser
	{
		...
		private void DoReduce(Token token, ReduceAction action)
		{
			...
			goldparser.lalr.Action gotoAction = currentState.Actions.Get(action.Rule.Lhs);
		...
		private void ParseTerminal(TerminalToken token)
		{
			...
			goldparser.lalr.Action action = currentState.Actions.Get(token.Symbol);
		...
		private SymbolCollection FindExpectedTokens()
		{
			...
			foreach(goldparser.lalr.Action action in state.Actions)
			{

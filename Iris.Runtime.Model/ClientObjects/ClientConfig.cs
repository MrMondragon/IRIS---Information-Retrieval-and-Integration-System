...
namespace Iris.Runtime.Model.ClientObjects
{
  ...
  public class ClientConfig : IProccessLog
  {
    ...
    public void SaveToFile(string _fileName)
    {
      ...
      foreach (IDataConnection connection in structure.Connections)
      {
    ...
    {
      ClientConfig config = PersistenceManager<ClientConfig>.LoadFromFile(_fileName,_structure.TypeLookupTable);

      config.fileName = _fileName;
      foreach (IDataConnection connection in _structure.Connections)
      {
    ...
    private void SaveConnectionSettings(IDataConnection connection)
    {
    ...
    private void LoadConnectionSettings(IDataConnection connection)
    {
    ...
    {
      return runningLog.Select(x => (ILogItem)x).ToList();
    }
  }
}

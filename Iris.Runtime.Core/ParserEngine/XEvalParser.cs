...
namespace Iris.Runtime.Core
{
  ...
  public class XEvalParser : BaseParser
  {
    ...
    public XEvalParser(IStructure aStructure)
      : this()
    {
      structure = aStructure;
    }

    public Object Parse(string source, Dictionary<string, object> _vars)
    {
      vars = _vars;
      return Parse(source);
    }
      ...
      switch (token.Rule.Id)
      {
        ...
        case (int)XEvalRuleConstants.RULE_DATATABLE_NUM_ID:
          //<DataTable> ::= '#' Id
          {
            ...
            {
              DataTable rs = structure.GetResultSet(GetText(token, 1)).Table;
              return rs;
            }
          }
        ...
        case (int)XEvalRuleConstants.RULE_FUNCTION_REQUESTOBJECT_LPARAN_ID_RPARAN:
          //<Function> ::= RequestObject ( Id )
          {
            ...
            {
              return structure.GetObject(GetText(token, 2));
            }
          }
    ...
    {
      IScalarVar var = structure.GetScalarVar(varName);
      if (var != null)
        return var.RawValue;
      else
        return null;

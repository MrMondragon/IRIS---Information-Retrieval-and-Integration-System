...
{
  [Serializable]
  [OperationCategory("Transformações", "Criar Colunas")]
  public class NewColumn : ColumnBasedOperation
  {
    ...
    protected override IEntity doExecute()
    {
      ...
      {
        DataTable newTable = new DataTable(Entrada.Name);
        if (Structure.Dataset.Tables.Contains(Entrada.Name))
          Structure.Dataset.Tables.Remove(Entrada.Name);

        Entrada.Table = newTable;     
      }
